...
class Model():
    def __init__(self, data_set, qos_ub) -> None:
        ...
        self.n_site, self.n_time, self.n_cust = data_set.n_site, data_set.n_time, data_set.n_cust # n_site 基站数量；n_time:时刻数; n_cust:用户数
        self.qos_ub = qos_ub # qos的上限
        ...
        self.w_max = self.m.addVars(self.n_site, lb=0, ub=GRB.INFINITY,vtype=GRB.INTEGER, name ='w_max')
    ...
    def set_qos_constrs(self):
        ...
        if self.qos[i,j] >= self.qos_ub), name ='qos')
    ...
    def set_demand_constrs(self):
        ...
        for t in range(self.n_time)), name = 'demand')
    
    def set_bandwidth_constrs(self):
        self.m.addConstrs((self.w[j,t] <= self.bandwidth[j] for j in range(self.n_site) for t in range(self.n_time)), name = 'bandwidth')
    ...
    def set_link_constrs(self):
        ...
        for t in range(self.n_time)), name ='link')
    
    def set_max_constrs(self):
        self.m.addConstrs((self.w_max[j] >= self.w[j,t] for j in range(self.n_site) for t in range(self.n_time) if t in self.sample_t), name = 'max')
